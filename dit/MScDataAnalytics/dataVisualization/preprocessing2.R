...
# Create the dataset by extracting samples from each downloaded dataset and save in a new CSV file
write.csv(uber_apr14[sample(1:nrow(uber_apr14), ds.sample.max.n), ], "uber_apr14.sampl.csv")
write.csv(uber_may14[sample(1:nrow(uber_may14), ds.sample.max.n), ], "uber_may14.sampl.csv")
write.csv(uber_jun14[sample(1:nrow(uber_jun14), ds.sample.max.n), ], "uber_jun14.sampl.csv")
write.csv(uber_jul14[sample(1:nrow(uber_jul14), ds.sample.max.n), ], "uber_jul14.sampl.csv")
write.csv(uber_aug14[sample(1:nrow(uber_aug14), ds.sample.max.n), ], "uber_aug14.sampl.csv")
write.csv(uber_sep14[sample(1:nrow(uber_sep14), ds.sample.max.n), ], "uber_sep14.sampl.csv")


# Cleanup uneeded variables to save memory
rm(uber_apr14, uber_may14, uber_jun14, uber_jul14, uber_aug14, uber_sep14)


DATEPARSE("MM/dd/yyyy HH:mm:ss", [Date/Time])